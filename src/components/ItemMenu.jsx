      ...
      {Food.map((item) => (
        <div key={item.id} className="lg:w-[280px] w[50%] my-5 hover:drop-shadow-lg rounded-lg h-[195px] bg-cardOverplay">
          <div className="flex justify-between items-center">
            ...
            <div onClick={()=>handleClickAddCart(item,user)} className={`w-7  hover:cursor-pointer h-7 flex justify-center items-center  bg-red-700 rounded-full lg:mx-3  ${animationState[item.id] ? 'motion-preset-expand motion-duration-500' : '' } ` }>
          ...
          <div className='flex w-full flex-col justify-end items-end'>
            ...
            <p className='px-3 pt-4 font-semibold lg:text-xl text-md'>
